...
class AttributeMatchingExtension extends AbstractExtension
{
    ...
    public function translateIncludes(XPathExpr $xpath, $attribute, $value)
    {
        return $xpath->addCondition($value ? sprintf(
            '%26$s and contains(concat(\' \', normalize-space(%26$s), \' \'), %2$s)',
            $attribute,
    ...
    public function translateDashMatch(XPathExpr $xpath, $attribute, $value)
    {
        return $xpath->addCondition(sprintf(
            '%26$s and (%26$s = %2$s or starts-with(%26$s, %3$s))',
            $attribute,
            Translator::getXpathLiteral($value),
            Translator::getXpathLiteral($value.'-')
        ));
    ...
    {
        return $xpath->addCondition($value ? sprintf(
            '%26$s and substring(%26$s, string-length(%26$s)-%2$s) = %3$s',
            $attribute,
            strlen($value) - 1,
            Translator::getXpathLiteral($value)
        ) : '0');
    ...
    {
        return $xpath->addCondition(sprintf(
            $value ? 'not(%26$s) or %26$s != %2$s' : '%s != %s',
            $attribute,
            Translator::getXpathLiteral($value)
        ));
    }
